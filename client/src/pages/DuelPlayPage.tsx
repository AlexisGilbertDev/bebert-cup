...
function computeNextChallenge(
  ...
  const candidates = isExhausted
    ? eligible.filter((c) => c.id !== excludeId)
    : pool;
...
export default function DuelPlayPage() {
  ...
  function nextRound() {
    ...
    if (isLastNormalRound) {
      ...
      const tieResult = computeNextChallenge(
        challenges,
        tied.length,
        usedChallengeIds,
      );
    ...
    const nextResult = computeNextChallenge(
      challenges,
      activePlayers.length,
      usedChallengeIds,
    );
  ...
  const isLastNormalRound = !isTieBreak && round === TOTAL_ROUNDS;
  const hasTieAfterLastRound =
    isLastNormalRound && findTied(scores, players).length > 1;
  const nextButtonLabel =
    isTieBreak || round < TOTAL_ROUNDS || hasTieAfterLastRound
      ? 'Manche suivante →'
      : 'Voir les résultats →';
          ...
          <>
              ...
              {!isTieBreak && (
                ...
                >
                  {ROUND_KEYS.map((key, i) => (
                    <div
                      key={key}
                      className={`dp-progress-segment${i < round ? ' dp-progress-segment--filled' : ''}`}
                    />
                  ))}
                </div>
              )}
            ...
            <div className="dp-ranking-section">
              <div className="dp-ranking-header">
                <div>
                  <p className="dp-ranking-title">
                    CLASSEZ DU GAGNANT AU PERDANT
                  </p>
                  <p className="dp-ranking-hint">
                    Touche les joueurs dans l'ordre — le 1er touché finit
                    premier.
                  </p>
                </div>
                <div className="dp-ranking-counter">
                  <span className="dp-counter-fraction">
                    {orderedRanks.length}/{activePlayers.length}
                  </span>
              ...
              <div className="dp-players">
                {activePlayers.map((player) => {
                  ...
                  const avatarColor =
                    PLAYER_COLORS[
                      players.indexOf(player) % PLAYER_COLORS.length
                    ] ?? '#888';
                  return (
                    <button
                      key={player}
                    ...
                    >
                      <span
                        className={`dp-medal${!isRanked ? ' dp-medal--empty' : ''}`}
                      >
              ...
              {orderedRanks.length > 0 && (
                <button
                  type="button"
                  className="dp-restart-btn"
                  onClick={() => setOrderedRanks([])}
                >
                        ...
                        style={{
                          ...
                          background: isLeader
                            ...
                            : index % 2 === 0
                              ...
                              : 'var(--paper)',
                          borderTop:
                            index === 0 ? 'none' : '2px solid var(--ink)',
                        }}
                      >
                        <span
                          style={{
                            font: '900 22px Nunito',
                            width: 32,
                            flexShrink: 0,
                          }}
                        >
                        ...
                        <span style={{ font: '800 16px Nunito', flex: 1 }}>
                          {player}
                        </span>
                        {gained > 0 && (
                          <span
